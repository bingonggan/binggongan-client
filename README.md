...
- [3. 핵심 기능 구현 방법](#3-%ED%95%B5%EC%8B%AC-%EA%B8%B0%EB%8A%A5-%EA%B5%AC%ED%98%84-%EB%B0%A9%EB%B2%95)
  - [3-1 빈 공간 없이 효율적으로 포장하기](#3-1-%EB%B9%88-%EA%B3%B5%EA%B0%84-%EC%97%86%EC%9D%B4-%ED%9A%A8%EC%9C%A8%EC%A0%81%EC%9C%BC%EB%A1%9C-%ED%8F%AC%EC%9E%A5%ED%95%98%EA%B8%B0)
    ...
    - [3D bin packing 알고리즘의 적용](#3d-bin-packing-%EC%95%8C%EA%B3%A0%EB%A6%AC%EC%A6%98%EC%9D%98-%EC%A0%81%EC%9A%A9)
...
- [4. 기능 개선](#4-%EA%B8%B0%EB%8A%A5-%EA%B0%9C%EC%84%A0)
  - [4-1 내 아이템의 위치를 알 수 있게 하기](#4-1-%EB%82%B4-%EC%95%84%EC%9D%B4%ED%85%9C%EC%9D%98-%EC%9C%84%EC%B9%98%EB%A5%BC-%EC%95%8C-%EC%88%98-%EC%9E%88%EA%B2%8C-%ED%95%98%EA%B8%B0)
  - [4-2 모든 물건을 담을 수 있게 개선하기](#4-2-%EB%AA%A8%EB%93%A0-%EB%AC%BC%EA%B1%B4%EC%9D%84-%EB%8B%B4%EC%9D%84-%EC%88%98-%EC%9E%88%EA%B2%8C-%EA%B0%9C%EC%84%A0%ED%95%98%EA%B8%B0)
...
**3D bin packing 알고리즘**은 크기, 모양, 무게가 다른 물체를 제한된 수의 3차원 컨테이너(빈)에 포장할 때, 공간 활용을 극대화하고 빈 공간을 최소화하기 위한 최적화 알고리즘입니다.

#### 3D bin packing 알고리즘 선정하기

3D bin packing 알고리즘은 주로 물체들을 부피에 따라 정렬하고 하나씩 배치해보는 **휴리스틱 기법**을 사용하여 구현됩니다.

휴리스틱 기법은 완벽한 답을 찾기보다는 빠르게 "좋은" 답을 찾는 방법입니다. 즉, 모든 경우를 다 계산하기보다는 경험적인 규칙이나 간단한 기준을 활용하여 최대한 효율적으로 상자를 채우는 방식입니다.

이 프로젝트에서 물체를 어떤 기준으로 정렬하는 것이 가장 적합할지 고민한 결과, **부피**와 **무게**를 정렬 기준으로 선택했습니다. 일반적으로 물건을 포장할 때 부피가 큰 것 중에서도 무게가 많이 나가는 물건을 먼저 포장하는 것이 효율적이라고 판단했기 때문입니다.

다음은 제가 검토한 다섯 가지 알고리즘과 각 특성의 비교입니다:

|                  | 1번 알고리즘 | 2번 알고리즘 | 3번 알고리즘 | 4번 알고리즘 | 5번<br> 알고리즘 |
| ---------------- | ------------ | ------------ | ------------ | ------------ | ---------------- |
| 회전 가능 여부   | **O**        | X            | O            | X            | X                |
| 무게 가중치 적용 | **O**        | X            | X            | O            | X                |
...
또한, 실린더 형태의 물건도 처리할 수 있어 다양한 물체를 다룰 수 있다는 점에서 프로젝트 요구사항에 가장 부합했습니다.

비교한 다른 알고리즘들은 각각 장단점이 있었지만, 회전 기능이 없거나 무게 가중치를 고려하지 않는 등의 이유로 이 프로젝트에서 사용하지 않았습니다.

#### 알고리즘의 적용

이 프로젝트에서 사용한 알고리즘은 OPTIMIZING THREE-DIMENSIONAL BIN PACKING THROUGH SIMULATION 논문을 토대로 구현되어 있습니다.

논문에는 알고리즘의 구현 방식과 알고리즘을 통해 얻을 수 있는 데이터가 명시되어 있는데요. 얻을 수 있는 데이터는 다음과 같습니다:
...
<div align="center">
  <img src="https://github.com/user-attachments/assets/7bb0941e-2606-4958-a090-f4e3b796d863" width="500rem"/>
...
<div align="center">
  <img src="https://github.com/user-attachments/assets/00236924-169a-4515-bd95-484169a91636" width="500rem"/>
...
모델의 색이 한 번 바뀌고 나서 다시 원래 색으로 돌아오지 않는 문제를 해결하기 위해, 처음에 모델을 불러올 때 원래 색 정보를 **userData**에 저장했습니다.

그 후 사용자가 특정 아이템을 클릭했을 때, 그 아이템과 같은 순번에 있는 모델의 색을 바꾸고, 다른 모델들은 원래 재질로 돌아가게 만들었습니다.
...
<div align="center">
  <img src="https://github.com/user-attachments/assets/b00de427-6389-491a-9783-5d610025c71c" width="400rem"/>

_내 아이템을 클릭하면 3D모델의 색이 변경됩니다._

</div>

### 4-2 모든 물건을 담을 수 있게 개선하기
...
> 서버의 계산량이 **물건의 개수**에 비례하여 증가함에 따라, 적절한 **물건 개수 제한**을 설정할 필요가 있었습니다. 이를 위해 **서버 부하 테스트**를 실시한 결과, **물건 개수를 15개로 제한**하는 것으로 결정하였습니다.
...
물건 20개 테스트부터 **약 10%의 요청에서 에러가 발생**되어 테스트를 중단하였습니다.

테스트 결과를 토대로 사용하고 있는 서버`AWS EC2 t2.micro`의 성능과 사용자 경험을 고려하여 **물건 개수를 15개**로 제한하였습니다.
